...
int checkFeastCard(int choice1,int coins, struct gameState *post, int p)
{
    ...
    if(cost <= playerCoins)//too expensive, player will be reprompted
    {
        if(supply > 0)//that card is in the supply, player can buy it.
        {
            ...
            //printf("choice %d\n",choice1);
            r = feastCard(post, p, temphand, choice1); // we would infinite loop if we didn't check first
            ...
            printf("handCount after feast: %d\n", post->handCount[p]);
            printf("discardCount after feast: %d\n", post->discardCount[p]);

            for(discardIndex = 0; discardIndex < post->discardCount[p]; discardIndex++) 
            {
                int card = post->discard[p][discardIndex];
                printf("card: %d\n",card);

                if(card == choice1);
                {
                    cardFound = true;
                    discardIndex = post->discardCount[p]; // break the loop
                }
            }
...
void createRandomDiscard(struct gameState *G, int discardCount, int p)
{
    int i;
...
int main()
{
    ...
    int choice1, p, coins, deckCount, handCount, discardCount;
    struct gameState G;
                    ...
                    for(discardCount = 1; discardCount < 5; discardCount ++)
                    {
                        for(coins = 0; coins < 5; coins++)
                        {
                           ...
                           // printf("coins:%d\n",coins);
                            ...
                            createRandomDiscard(&G, discardCount, p);
                            printf("DiscardCount Initial:%d\n",discardCount);
                            if(coins > 0)
                            {
